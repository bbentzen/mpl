...
import ..default data.encodable .syntax.language.basic
...
| (bot σ) := dec_trivial
| (p ⊃ q) := by apply nat.le_trans (succ_le_succ (zero_le 1)); apply prime_double_exp_le prime_five prime_seven (encode_ge_one p) (encode_ge_one q)
| (◻p) :=
  by apply nat.le_trans (succ_le_succ (zero_le 1)); unfold encode_form; cases le.dest (encode_ge_one p);
